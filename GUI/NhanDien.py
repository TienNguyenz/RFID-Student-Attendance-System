...
class UI_NhanDien(object):
        ...
        hinhanh = ''
        arr_masinhvien = []
        isDiemDanhComplete = False  # Flag để kiểm tra trạng thái điểm danh
        flagSetInfoSV = False
        ...
        "}")
                ...
                self.imageNhanDien.hide()  # Ẩn hình ảnh
        ...
        def retranslateUi(self, MainWindow):
                ...
                self.label_3.setText(_translate("MainWindow", "Quét bằng thẻ sinh viên"))
        ...
        def show_wedcam(self, cv_img):
                """Quét và nhận diện mã vạch từ webcam"""
                ...
                for obj in decoded_objects:
                        ...
                        self.masinhvien = "SV" + barcode_data  # Thêm tiền tố "SV" vào mã sinh viên

                        # Kiểm tra nếu mã sinh viên chưa được thêm vào danh sách
                        if self.masinhvien not in self.arr_masinhvien:
                                self.arr_masinhvien.append(self.masinhvien)
                                self.flagSetInfoSV = False  # Chưa cập nhật thông tin sinh viên

                        # Nếu mã sinh viên hợp lệ và chưa được cập nhật thông tin
                        if self.masinhvien != '' and not self.flagSetInfoSV:

                                print(f"Debug: Đang gọi setInfoSV cho mã sinh viên: {self.masinhvien}")
                                self.setInfoSV()
                                print("Debug: Hoàn thành setInfoSV")
                                if self.cmbLoaiDiemDanh.currentIndex() == 1:
                                        print("Debug: Đang gọi addDiemDanh")
                                        self.addDiemDanh()
                                        print("Debug: Hoàn thành addDiemDanh")
                                else:
                                        self.updateGioRa()
                                self.flagSetInfoSV = True  # Đánh dấu đã cập nhật thông tin sinh viên
        ...
        def setInfoSV(self):
                if self.masinhvien != '':
                        print(f"Debug: Mã sinh viên: {self.masinhvien}")
                        tensinhvien = ''
                        svBUS = SinhVienBUS()

                        # Tìm sinh viên theo mã vạch
                        ...
                        if tensinhvien != '':
                                print(f"Debug: Tên sinh viên tìm được: {tensinhvien}")

                                # Cập nhật các trường thông tin sinh viên lên UI
                                self.txtMaSV.setText(self.masinhvien)
                                self.txtHoTen.setText(tensinhvien)

                                # Debug thông tin trước khi cập nhật thời gian
                                print(f"Debug: Thời gian hiện tại trên label_2: {self.label_2.text()}")
                                ...
                                try:
                                        ...
                                        print(f"Debug: Thời gian đã được set: {time}")
                                except Exception as e:
                                        print(f"Lỗi khi chuyển đổi thời gian: {e}")
                        else:
                                print(f"Debug: Không tìm thấy sinh viên với mã {self.masinhvien}")
                else:
                        print("Debug: Mã sinh viên rỗng!")

        def addDiemDanh(self):
                ddBUS = DiemDanhBUS()
                madiemdanh = ddBUS.generateID()
                masinhvien = self.masinhvien  # Sử dụng mã vạch
                ...
                # Gọi phương thức add từ BUS để lưu vào database
                result = ddBUS.add(diemdanh)
                if result:
                        print(f"Debug: Điểm danh {masinhvien} đã được thêm thành công!")
                        QMessageBox.information(self.centralwidget, "Thông báo", "Thêm điểm danh thành công!")

                else:
                        print(f"Debug: Thêm điểm danh thất bại cho {masinhvien}")
                        QMessageBox.warning(self.centralwidget, "Thông báo", "Thêm điểm danh thất bại!")
        ...
        def updateGioRa(self):
                ...
                QMessageBox.information(self.centralwidget, "Thông báo", "Cập nhật giờ ra thành công!")
                self.isDiemDanhComplete = False
