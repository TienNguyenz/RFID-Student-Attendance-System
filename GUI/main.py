...
class mainGUI():
    ...
    def listChangeStyleSheet_MD(self):
        ...
        # Đảm bảo rằng bạn sử dụng đúng đường dẫn đến tệp .qss
        qss_file_path = Path("../qss/py_dark_style.qss")

        # Kiểm tra nếu tệp tồn tại trước khi đọc
        if qss_file_path.exists():
            with qss_file_path.open("r") as f:
                self.app.setStyleSheet(f.read())
        else:
            print(f"Tệp {qss_file_path} không tồn tại!")
    ...
    def listChangeStyleSheet_Dark(self):
        ...
        # Đảm bảo rằng bạn đang đọc tệp từ đường dẫn đúng
        qss_file_path = Path(r"../qss/py_dark_style.qss")

        # Kiểm tra nếu tệp tồn tại trước khi đọc
        if qss_file_path.exists():
            self.app.setStyleSheet(qss_file_path.read_text())
        else:
            print(f"Tệp {qss_file_path} không tồn tại!")
    ...
    def mainUi(self, MainWindow ,page):
        ...
        # dd su kien
        self.ui.btnDD.clicked.connect(self.DdSukien)
        # Chọn nhận diện
    ...
    def DdSukien(self):
        self.ui = Diemdanhsukien.ImageScannerUI()
        self.MainWindow = MainWindow
        self.ui.setupUi(MainWindow)
