...
"}")
        ...
        def getDataRow(self):
            try:
                ...
                # Lấy thời gian vào và ra từ bảng
                time_str_Vao = self.tbwDiemDanh.item(cr, 4).text()
                time_str_Ra = self.tbwDiemDanh.item(cr, 5).text()

                # Kiểm tra nếu thời gian vào là None hoặc rỗng, thay bằng "00:00:00"
                if not time_str_Vao or time_str_Vao == 'None':
                    time_str_Vao = "00:00:00"  # Nếu None hoặc chuỗi rỗng, gán giá trị mặc định là "00:00:00"

                # Kiểm tra nếu thời gian ra là None hoặc rỗng, thay bằng "00:00:00"
                if not time_str_Ra or time_str_Ra == 'None':
                    time_str_Ra = "00:00:00"  # Nếu None hoặc chuỗi rỗng, gán giá trị mặc định là "00:00:00"
                ...
                print(f"Debug: Lỗi khi lấy dữ liệu: {e}")
        ...
        def importExcel(self):
            ...
            if dir_path != "":
                QuestionDel = QMessageBox.question(self.centralwidget, 'Thông báo',
                                                   "Thao tác này sẽ làm thay đổi dữ liệu cũ thành dữ liệu mới. Bạn có muốn tiếp tục?",
                ...
                if QuestionDel == QMessageBox.StandardButton.Yes:
                    ...
                    sheet = wb.active
                    # Lặp qua từng dòng của sheet Excel
                    for r in range(2, sheet.max_row + 1):
                        ...
                        hinhanh = sheet.cell(r, 8).value if sheet.cell(r,
                                                                       8).value else "default_image_path.jpg"  # Nếu không có giá trị hình ảnh thì sử dụng giá trị mặc định

                        # Tạo đối tượng DiemDanh với đầy đủ tham số
                        dd = DiemDanh(madiemdanh, masinhvien, giovao, giora, mabuoihoc, hinhanh)
        ...
        def exportExcel(self):
                ...
                tenfile =  r"D:\Study\Nam4\Python\OpenCV\Doan\ProjectPython-FaceID-main\FileExcel\DiemDanhDiemDanh{}.xlsx".format(current_time)
