...
class UI_QuanLyTaiKhoan(object):
        ...
        "background-color:qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 rgba(100, 170, 230, 255), stop:1 rgba(120, 180, 230, 255));\n"
        "color:white;\n"
        "font-weight:bold;\n"
        "border-radius:3px;\n"
        "padding: 5px;\n"
        ...
        "background-color: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 rgba(150, 200, 255, 255), stop:1 rgba(120, 180, 230, 255)); \n"
        "}")
        ...
        def addTaiKhoan(self):
                ...
                try:
                        print(f"Kết quả kiểm tra email: {email}")
                        ktraEmail = tkBUS.checkEmailTonTai(email)
                        print(f"Kết quả kiểm tra email: {ktraEmail}")  # Debug kết quả kiểm tra

                        # Kiểm tra nếu ktraEmail là True, tức là email đã tồn tại
                        if ktraEmail:
                                QMessageBox.information(self.centralwidget, "Thông báo", "Email đã tồn tại")
                                print("Thất bại: Email đã tồn tại.")
                        else:
                                print("Email chưa tồn tại, tiến hành thêm tài khoản.")
                                tk = TaiKhoan(mataikhoan, email, matkhau, quyen)
                                if self.validateTK(tk):  # Kiểm tra dữ liệu tài khoản
                                        if tkBUS.add(tk):  # Thêm tài khoản vào cơ sở dữ liệu
                                                print(f"Tài khoản {email} đã được thêm thành công.")
                                                QMessageBox.information(self.centralwidget, "Thông báo",
                                                                        "Thêm thành công")
                                                self.loadDataQTableTaiKhoan()
                                                self.clearTaiKhoan()
                                        else:
                                                print(f"Thêm tài khoản {email} thất bại.")
                                                QMessageBox.information(self.centralwidget, "Thông báo",
                                                                        "Thêm thất bại")
                except Exception as e:
                        print(f"Lỗi khi thêm tài khoản: {e}")
                        QMessageBox.information(self.centralwidget, "Thông báo", "Đã xảy ra lỗi khi thêm tài khoản.")
        ...
        def validateTK(self, tk: TaiKhoan):
                ...
                # Gọi checkEmailTonTai từ TaiKhoanBUS
                if TaiKhoanBUS.checkEmailTonTai(tk._email):
                        QMessageBox.information(self.centralwidget, "Thông báo", "Email đã tồn tại")
                        return False
