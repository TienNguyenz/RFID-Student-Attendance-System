...
class UI_ThongKe(object):
        ...
        def filedimuon(self):
                ...
                for index, sv in enumerate(self.listDiMuon):
                        # Debug: Kiểm tra dữ liệu trong self.listDiMuon
                        print(f"Debug: Danh sách sinh viên đi muộn tại index {index}: {sv}")
                ...
                try:
                        df = pd.DataFrame(x)
                        df.to_excel(r"D:\Study\Nam4\Python\OpenCV\Doan\ProjectPython-FaceID-main\FileExcel\ThongKe\Danhsach_SVDiMuon{}.xlsx".format(current_time),
                                    engine='xlsxwriter', index=False)
                        QMessageBox.information(self.centralwidget, "Thông báo",
                                                "Xuất danh sách sinh viên đi muộn thành công!")
                except Exception as e:
                        print(f"Debug: Lỗi khi xuất file danh sách sinh viên đi muộn: {e}")

        def filevang(self):
                x = {'Mã SV': [], 'Họ Tên': [], 'Lớp': [], 'Ngày': [], 'ID buổi học': []}
                for index, sv in enumerate(self.listVang):
                        # Debug: Kiểm tra dữ liệu trong self.listVang
                        print(f"Debug: Danh sách sinh viên vắng tại index {index}: {sv}")
                ...
                try:
                        df = pd.DataFrame(x)
                        df.to_excel(r"D:\Study\Nam4\Python\OpenCV\Doan\ProjectPython-FaceID-main\FileExcel\ThongKe\Danhsach_SVVang{}.xlsx".format(current_time),
                                    engine='xlsxwriter', index=False)
                        QMessageBox.information(self.centralwidget, "Thông báo",
                                                "Xuất danh sách sinh viên vắng thành công!")
                except Exception as e:
                        print(f"Debug: Lỗi khi xuất file danh sách sinh viên vắng: {e}")

        def fileKDD(self):
                x = {'Mã SV': [], 'Họ Tên': [], 'Lớp': [], 'Ngày': [], 'ID buổi học': []}
                for index, sv in enumerate(self.listKDD):
                        # Debug: Kiểm tra dữ liệu trong self.listKDD
                        print(f"Debug: Danh sách sinh viên không điểm danh tại index {index}: {sv}")
                ...
                try:
                        df = pd.DataFrame(x)
                        df.to_excel(r"D:\Study\Nam4\Python\OpenCV\Doan\ProjectPython-FaceID-main\FileExcel\ThongKe\Danhsach_SVKhongDiemDanh{}.xlsx".format(current_time),
                                    engine='xlsxwriter', index=False)
                        QMessageBox.information(self.centralwidget, "Thông báo",
                                                "Xuất danh sách sinh viên không điểm danh thành công!")
                except Exception as e:
                        print(f"Debug: Lỗi khi xuất file danh sách sinh viên không điểm danh: {e}")
