...
class GiangVienDAL:
    ...
    def checkExistTaiKhoan(mataikhoan):
        try:
            ...
            conn = connDb.Connect()
            cursor = conn.cursor()
            query = "SELECT * FROM giangvien WHERE mataikhoan = '{}'".format(mataikhoan)
            cursor.execute(query)
            row = cursor.fetchone()
            print(f"Kiểm tra tài khoản '{mataikhoan}': Kết quả query - {row}")

            if row is None:
                return True  # Tài khoản chưa tồn tại
        except Exception as ex:
            print("Lỗi trong checkExistTaiKhoan:", ex)
        return False  # Tài khoản đã tồn tại hoặc có lỗi
    ...
    def checkSoDienThoaiTonTai(sodienthoai):
        try:
            ...
            cursor.execute(query)
            row = cursor.fetchone()
            print(f"Kiểm tra số điện thoại '{sodienthoai}': Kết quả query - {row}")

            if row is None:
                return True  # Số điện thoại không tồn tại
        except Exception as ex:
            print("Lỗi trong checkSoDienThoaiTonTai:", ex)
        return False  # Số điện thoại đã tồn tại hoặc có lỗi
