...
class SinhVienDAL:
    ...
    def generateID():
        ...
        ma = "SV{0:010}".format(stt)
        return ma

    @staticmethod
    def updateMSSV(old_mssv, new_mssv):
        """
        Cập nhật MSSV trong cơ sở dữ liệu.
        """
        try:
            # Ví dụ cập nhật trong cơ sở dữ liệu
            query = f"UPDATE sinhvien SET masinhvien = '{new_mssv}' WHERE masinhvien = '{old_mssv}'"
            connDb = ConnectDatabase()
            conn = connDb.Connect()
            cursor = conn.cursor()
            cursor.execute(query)
            conn.commit()
            print(f"Đã cập nhật MSSV từ {old_mssv} sang {new_mssv}")
            return True
        except Exception as e:
    ...
    def checkTonTai(key, value):
        try:
            ...
            query = "SELECT * FROM sinhvien WHERE {} = %s".format(key)
            cursor.execute(query, (value,))
            row = cursor.fetchone()

            print(f"Kiểm tra {key}='{value}': Kết quả query - {row}")

            if row is None:  # Không tìm thấy dòng nào
                return False
            return True  # Tìm thấy dòng
        except Exception as ex:
            print("Lỗi trong checkTonTai:", ex)
            return False
