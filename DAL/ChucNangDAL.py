...
class ChucNangDAL:
    ...
    def checkTenCNTonTai(tenchucnang):
        try:
            ...
            FROM chucnang
            WHERE tenchucnang = %s """
            cursor.execute(query, (tenchucnang,))  # Sử dụng tham số hóa trong câu truy vấn để tránh SQL injection
            row = cursor.fetchone()

            if row is None:  # Nếu không có kết quả, trả về True (không tồn tại)
                return True
        except Exception as ex:
            print(ex)
        return False
